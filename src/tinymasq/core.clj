...
(defn create-udp-socket []
  (DatagramSocket. (tiny-config :udp-port) (. InetAddress getByName (tiny-config :bind-address))))

(defn start-udp-server []
  (reset! udp-server (create-udp-socket)))
...
(defn setup-logging
  ...
  []
  (timbre/set-config! [:shared-appender-config :spit-filename] (log-conf :path))
